...
class UpdateAnnouncement(RetrieveUpdateDestroyAPIView):
    permisson_classes = [IsAuthenticated, IsNeedyActive, IsNeedyOwner]
    serializer_class = AnnouncementSerializer 
    lookup_field="pk"
...
class UpdateProduct(APIView):
    permisson_classes = [IsAuthenticated, IsNeedyActive, IsNeedyOwner, IsProductNotAssigned]
    ...
    def put(self, request, product_pk, format='json'):
    ...
    def delete(self, request, pk, product_pk, format='json'):
        product = self.get_object(product_pk)
        product.delete()
...
class AssignProduct(APIView):
    permisson_classes = [IsAuthenticated, IsVolunteerActive, IsProductNotAssigned]

    def put(self, request, pk, product_pk, format='json'):
        product = Product.objects.get(product_id=product_pk)
        volunteer = Volunteer.objects.get(user=request.user) 
        product.volunteer = volunteer
        product.save()
        return Response({'product' : 'Successfully assigned to volunteer'}, status = status.HTTP_200_OK)

    
class ListMyAssignedProduct(ListAPIView):
    permisson_classes = [IsAuthenticated, IsVolunteerActive]
    serializer_class = ProductSerializer 
